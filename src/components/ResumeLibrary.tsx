...
export function ResumeLibrary() {
  ...
  const importMutation = useMutation({
    mutationFn: (data: { name: string; content: string; format: 'markdown' | 'pdf' }) => 
            ...
            <div className="space-y-4 flex-1 flex flex-col min-h-0">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
                  {importMode === 'pdf' ? 'PDF Attached' : 'Resume Content (Markdown)'}
                </span>
                {importMode === 'pdf' && (
              ...
              {importMode === 'markdown' ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center bg-muted/20 rounded-xl border border-dashed border-border min-h-[200px]">
                  <FileText className="h-12 w-12 text-muted-foreground/40 mb-2" />
                  <p className="text-sm text-muted-foreground">PDF file ready for parsing</p>
                  <Button variant="ghost" size="sm" className="mt-4" onClick={() => setImportingMode(null)}>Change File</Button>
                </div>
              )}
    ...
    <div className="h-full flex flex-col bg-muted/5">
      <div className="p-4 border-b border-border/40 flex items-center justify-between bg-background/80 backdrop-blur-md sticky top-0 z-10">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">{resumes.length} versions stored</p>
        </div>
        <div className="flex gap-2">
          <Button 
            ...
            onClick={() => window.open(window.location.href.split("#")[0] + "#/workspace", "_blank")}
          >
            <ExternalLink className="h-3.5 w-3.5" />
          </Button>
          <Button size="sm" className="h-8 rounded-lg px-3" onClick={() => setIsImporting(true)}>
        ...
        {isLoading ? (
          ...
          <div className="h-[50vh] flex flex-col items-center justify-center text-center text-muted-foreground space-y-4 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div>
              ...
              <p className="text-xs max-w-[200px] mt-1">Import your resume to start analyzing jobs and tailoring applications.</p>
            </div>
          </div>
        ) : (
          resumes.map((resume) => (
              ...
              <CardContent className="p-3 relative">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${resume.isDefault ? 'bg-primary/10 text-primary' : 'bg-muted/50 text-muted-foreground'}`}>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                        <span className="capitalize">{resume.source.replace('_', ' ')}</span>
                        <span>•</span>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-40 rounded-xl">
                      {!resume.isDefault && (
                        <DropdownMenuItem className="text-xs py-2 rounded-lg" onClick={() => setDefaultMutation.mutate(resume.id)}>
                ...
                <div className="mt-3 flex items-center justify-between text-[10px] font-medium pt-3 border-t border-border/30">
                  <span className="text-muted-foreground">Used {resume.useCount} times</span>
                  <div className="flex items-center gap-2">
                    {resume.isDefault && (
                      <Badge variant="secondary" className="h-5 text-[9px] bg-primary/5 text-primary border-primary/10 px-1.5">
                        DEFAULT
                      </Badge>
                    )}
                  </div>
